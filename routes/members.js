    ...
    async.waterfall([
      function (next){
        node.hdc.amendments.current(function (err, am) {
          if(am){
            node.hdc.amendments.view.memberships(am.number, am.hash, function (err, mems) {
              next(err, mems || []);
            });
          }
          else next(null, []);
      ...
      function (mems, next){
        async.forEach(mems, function(item, callback){
          status[item.fingerprint] = { membership: item };
          node.pks.lookup('0x' + item.fingerprint, function (err, keys) {
            if(keys.length == 0){
              callback('Member\'s public key with FPR ' + '0x' + item.fingerprint + ' not stored!');
              return;
            }
            status[item.fingerprint].key = keys[0];
            callback(err);
          });
        }, next);
      }
